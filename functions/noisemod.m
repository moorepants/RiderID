function H = noisemod(npm)
    ...
    H = [X(1)*(1+X(2)*s)/(1 + X(3)*s)^4*(1+X(4)*s)^3 ...
         X(5)*(1+X(6)*s)/(1 + X(7)*s)^6*(1+X(8)*s)^5;]; 
     
%     rem.Hd = c2d(rem.H,1/200);
%     disp(isstable(1/rem.Hd(1)));
%     disp(isstable(1/rem.Hd(2)));

%     rem.V_mod = V_modfunc(X,npm.f);
%     V_mod = V_modfunc(X,f);
...
function V_mod = V_modfunc(X,f)
    ...
    V_mod = [X(1).*(1+X(2).*s)./(1 + X(3).*s).^4.*(1+X(4).*s).^3 ...
             X(5).*(1+X(6).*s)./(1 + X(7).*s).^6.*(1+X(8).*s).^5;];    
